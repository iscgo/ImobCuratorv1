...
export const ClientPortal: React.FC = () => {
  ...
  const stats = {
    ...
    active: MOCK_CLIENTS.filter(c => c.status === 'Searching' || c.status === 'Visiting').length,
    attention: MOCK_CLIENTS.filter(c => c.attentionNeeded).length,
    closed: MOCK_CLIENTS.filter(c => c.status === 'Closed').length
  };
