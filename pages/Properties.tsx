...
export const Properties: React.FC = () => {
  ...
  // Initialize from MOCK_PROPERTIES to ensure we see updates from PropertyDetail
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Tem a certeza que deseja remover este imóvel do portfólio?')) {
      ...
      // Update the "Global" mock for persistence in this demo
  ...
  const handleSmartExtract = async () => {
    ...
    try {
        ...
        const prompt = `
            Analyze this real estate listing URL: ${extractionUrl}
            
            Task: Simulate visiting this URL and extracting the property details.
            
            Return a JSON object with these fields (guess based on the URL structure or common listings if you can't browse live):
            - title (string)
            ...
            - bathrooms (number)
            - area (number, m2)
            - imageUrl (string, a realistic placeholder or the real one if found in metadata)
        ...
        if (text) {
            ...
            setNewProperty({
                ...
                area: data.area || 0,
                // Fallback image if AI returns empty or null
        ...
        alert("Não foi possível extrair automaticamente. Por favor preencha manualmente.");
    } finally {
  ...
  const handleAddProperty = (e: React.FormEvent) => {
    ...
    // Add to local state and global mock
