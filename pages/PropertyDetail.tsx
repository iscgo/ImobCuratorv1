...
export const PropertyDetail: React.FC = () => {
  ...
  const [currentStatus, setCurrentStatus] = useState<PropertyStatus>(property ? property.status : PropertyStatus.NEW);
  ...
  const handleSaveNote = () => {
      ...
      // For demo, we update the Mock reference
      if (propertyIndex > -1) {
          MOCK_PROPERTIES[propertyIndex].agentNote = agentNote;
      }
      setIsEditing(false);
  };
  ...
  const updateStatus = (newStatus: PropertyStatus) => {
      ...
      if (propertyIndex > -1) {
          MOCK_PROPERTIES[propertyIndex].status = statusToSet;
      }
  };
