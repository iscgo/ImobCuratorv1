...
import { MOCK_PROPERTIES, MOCK_CLIENTS, CURRENT_USER } from '../constants';
...
export const PropertyImport: React.FC = () => {
  ...
  const handleSearch = async (append: boolean = false) => {
    ...
    try {
      ...
      // Prompt Refinado para Filtrar Imóveis Vendidos e Priorizar Disponibilidade
      const prompt = `
        DATA ATUAL: ${today}.
        
        MISSÃO CRÍTICA: Encontrar 15 imóveis que estejam ATIVAMENTE À VENDA AGORA em Portugal.
        
        ⚠️ FILTRO DE SEGURANÇA (MUITO IMPORTANTE):
        1. EXCLUA IMEDIATAMENTE qualquer imóvel marcado como "Vendido", "Sold", "Reservado", "Sob Oferta" ou "Indisponível".
        2. VERIFIQUE A DATA: Priorize anúncios publicados ou atualizados nos últimos 30 dias. Evite anúncios antigos de 2024 que já não existem.
        3. DIVERSIFIQUE: Não use apenas o Idealista. Pesquise obrigatoriamente em: Imovirtual, SuperCasa, Casa.sapo.pt, BPI Expresso Imobiliário, Sites oficiais de agências (Remax.pt, KWPortugal.pt, Era.pt, Zome.pt).

        CRITÉRIOS DO CLIENTE:
        - Tipo: ${criteria.type}
        - Localização: ${criteria.location}
        - Preço Máximo: ${criteria.budgetMax ? '€' + criteria.budgetMax : 'Sem limite'}
        - Configuração: T${criteria.bedrooms}+, ${criteria.bathrooms}+ WC
        - Área Mínima: ${criteria.areaMin ? criteria.areaMin + ' m²' : 'Indiferente'}
        - Obrigatório: ${amenities.join(", ")}
        - Extras: ${criteria.otherRequirements}

        AÇÃO:
        1. Utilize o Google Search para encontrar anúncios reais.
        2. Certifique-se que o link leva a uma página de produto/anúncio e não a uma lista genérica de pesquisa.
        3. Ignore estes links já encontrados: ${JSON.stringify(excludedUrls)}.
        4. Encontre 15 opções.

        FORMATO DE RESPOSTA (JSON Array estrito com 15 itens):
        [
          {
            "title": "Título do anúncio",
            "price": "Preço (ex: €450.000)",
            "location": "Localização",
            "url": "Link direto VÁLIDO",
            "matchScore": 95, 
            "matchReason": "Motivo curto do match.",
            "pros": ["Ponto positivo 1", "Ponto positivo 2"],
            "cons": ["Ponto de atenção ou 'Nenhum'"]
          }
        ]
      `;
      ...
      if (text) {
        ...
        // Sanitização agressiva de Markdown
        cleanJson = cleanJson.replace(/```json/g, '').replace(/```/g, '');

        try {
            const matches = JSON.parse(cleanJson) as PropertyMatch[];
            
            if (matches && matches.length > 0) {
                // Filtrar resultados sem URL ou com URLs inválidas
                const validMatches = matches.filter(m => m.url && m.url.startsWith('http'));
                
                // Atualizar lista de exclusão para a próxima busca
                ...
                setSearchStatus('Não foram encontrados imóveis ativos com estes critérios exatos. Tente ampliar a zona ou o orçamento.');
            }
        } catch (jsonError) {
            console.error("Erro ao processar JSON:", jsonError);
            setSearchStatus('Erro ao processar os dados. Tente novamente.');
        }
      } else {
          setSearchStatus('O agente não respondeu. Verifique a conexão.');
      }

    } catch (error) {
      console.error("Erro na busca:", error);
      setSearchStatus('Erro de conexão com a IA. Tente novamente.');
    } finally {
  ...
  const handleAddToPortfolio = (match: PropertyMatch) => {
    // 1. Garantir que o Cliente Existe no "Banco de Dados" (MOCK_CLIENTS)
    let targetClient = MOCK_CLIENTS.find(c => c.name.toLowerCase() === clientDetails.name.toLowerCase());
    ...
    if (!targetClient) {
        ...
        targetClient = {
            ...
            status: 'Offer Made', // Define status como "Proposta Enviada" para aparecer ativo
            lastActivity: 'Agora mesmo',
            unreadCount: 0,
            proposalSent: true,
            attentionNeeded: false
        };
        // Adicionar ao topo da lista de clientes
        MOCK_CLIENTS.unshift(targetClient);
    } else {
    ...
    const newProperty: Property = {
        ...
        imageUrl: `https://picsum.photos/seed/${match.title.length}/800/600`, // Placeholder
        url: match.url,
    ...
    // 4. Add to Global Properties List
    MOCK_PROPERTIES.unshift(newProperty);
    
    // 5. Update UI State
    setAddedProperties(prev => new Set(prev).add(match.url));
    
    // Feedback visual opcional (console ou toast se tivéssemos)
