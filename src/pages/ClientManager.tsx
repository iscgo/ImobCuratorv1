...
export const ClientManager: React.FC = () => {
  ...
  useEffect(() => {
    setClientProperties(getProperties());
  }, [id]);
  ...
  const handleUpdateProposal = async () => {
    ...
    try {
      const context = `O cliente descartou ${discardedProperties.length} imóveis e não gostou de nenhum. Preciso reconhecer que as opções anteriores não serviram, pedir feedback e sugerir realinhamento da busca.`;

      const message = await aiService.generateClientMessage(
        client.name,
        context,
        'reengagement'
      );

      setGeneratedMessage(message || "Não foi possível gerar a mensagem.");
  ...
  const handleGenerateAgentMessage = async (propId: string) => {
      ...
      try {
          const message = await aiService.generateAgentMessage(
              listingAgent.name,
              listingAgent.agency,
              CURRENT_USER.name,
              CURRENT_USER.agency,
          ...
          setAgentMessage(message || "Erro ao gerar mensagem.");
  ...
  const handleBroadcastMessage = async () => {
      ...
      try {
          const message = await aiService.generateClientMessage(
              client.name,
              `Broadcast para múltiplos agentes sobre cliente com orçamento ${client.budget} interessado em ${client.locationInterest}`,
              'proposal'
          );
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/20"
                    >
